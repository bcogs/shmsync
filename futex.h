...
inline bool FutexSyscall(volatile uint32_t* addr, int futex_op, uint32_t val) {
...
// will wake it up is worked around.
  ...
  return syscall(SYS_futex, addr, futex_op, val, 0, NULL, 0) != -1 || errno == EAGAIN;
#else
  timespec timeout;
  timeout.tv_sec = 0;
  timeout.tv_nsec = 100 * 1000;
  return syscall(SYS_futex, addr, futex_op, val, &timeout, NULL, 0) != -1 || errno == EAGAIN || errno == ETIMEDOUT;
#endif
}
